...
class TextReader(object):
    ...
    def encode_one_hot(self, sentence):
        ...
        sent = []
        SENT_LENGTH = 0
        encoded_sentence = list(filter(lambda x: x in (printable), sentence))  
        encoded_sentence_string = ''.join(encoded_sentence)      
        
        #for word in word_tokenize(encoded_sentence.decode('utf-8', 'ignore').encode('utf-8')):
        for word in word_tokenize(encoded_sentence_string):
    ...
    def make_minibatch(self, sentences):
        ...
        for sentence in sentences:
            ...
            if length == 0:
                one_hot, length = self.encode_one_hot('Blank Line')
