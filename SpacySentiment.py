...
def convert_scale(original_list):
    aggregate = 0.0
    for item in original_list:
        aggregate += item
    average = aggregate / len(original_list)
    if average > 1.0:
        average = 1.0
    elif average < -1.0:
        average = -1.0
...
class SentimentAnalyser(object):
    ...
    def pipe(self, docs, batch_size=1000, n_threads=2):
        for minibatch in toolz.itertoolz.partition_all(batch_size, docs):
...
def evaluate_without_labels(nlp, texts, out_file=None):
    sentiments = []
        ...
        with open(out_file, mode='w', encoding="utf-8") as file:
            for doc in nlp.pipe(texts, batch_size=1000, n_threads=4):
                try:
                    ...
                    # if there is not enough to determine, then assume english as a default behaviour
                    pass
                file.write(str(doc.sentiment) + '\n')
                file.write(doc.text + '\n')            
                sentiments.append(doc.sentiment)
    keras.backend.clear_session()
    # we are forcing document only analysis for now
    return convert_scale(sentiments)
...
def benchmark_test():
    ...
    print('Getting data')    
    directory = os.fsencode('input/train')
    for file in os.listdir(directory):
        with open(os.path.join(directory, file), mode='r', encoding="utf-8") as file:
            data = file.read()
            # we don't have predictions, so just say for 0 for everything
            this_list = [data, 0]
            bm_data.append(this_list)

    print('Zipping data')
    texts, labels = zip(*bm_data)
    print('Classifying')
    nlp = spacy.load('en_vectors_web_lg')
    nlp.add_pipe(nlp.create_pipe('sentencizer'))
    nlp.add_pipe(SentimentAnalyser.load(pathlib.Path('model'), nlp, max_length=100))
    evaluate_without_labels(nlp, texts, 'results.txt')
...
if __name__ == '__main__':
    ...
    for i in range(0, len(filelines)):
        ...
        document = '\n'.join(fileline)
        result = evaluate_without_labels(nlp, document, 'results.txt')
        print(result)